...
namespace RichsRack.Features.Transactions
{
	public class GetTransactionsQueryHandler: IQueryHandler<GetTransactionsQuery, IQueryable<Transaction>>
	{
        ...
        public GetTransactionsQueryHandler(SnacksDbContext dbContext, IDistributedCache cache, IConfiguration configuration)
		{
        ...
        {
            var transactions = dbContext.Transactions.FromSqlRaw("CALL GetAllTransactions()").AsCachedQueryable(_cache, TransactionsKey);
